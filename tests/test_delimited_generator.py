...
def test_tab_delimited_generator_hits_target(tmp_path) -> None:
    ...
    generator = DelimitedFileGenerator()
...
def test_csv_generator_uses_comma_delimiter(tmp_path) -> None:
    destination = tmp_path / "large.csv"
    headers = ("FirstName", "LastName")
    row_generator = DefaultRowContentGenerator(filler_text="Data", seed="csv-seed", digest_length=16)
    constraint = SizeConstraint(target_bytes=2_000, tolerance_bytes=256)
    ...
    generator = DelimitedFileGenerator()
    generator.generate(request, _collect_progress([]))

    assert destination.exists()
    with destination.open("r", encoding="utf-8") as handle:
        header_line = handle.readline().strip()
        blank_line = handle.readline().strip()
        first_data = handle.readline().strip()

    assert header_line == ",".join(headers)
    assert blank_line == ""
    assert "," in first_data and "\t" not in first_data
