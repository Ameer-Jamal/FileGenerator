...
6. The app remembers your last path, headers, filler token, and sizing so repeated test runs start with the same setup.
7. Estimates update live—showing required disk space, current free space, and a rough run-time—before you click **Generate**. The generator blocks when the destination volume lacks enough room.
